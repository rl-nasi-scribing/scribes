\section{Bellman Equation, Bellman Error, and Projected Bellman Error}
...
The \emph{Bellman error} measures how well a candidate value function $v$ satisfies the Bellman equation. In single-state form:
\[
\text{Bellman Error}(s) 
...
% \subsection{Summary}
% \begin{itemize}
%     \item The Bellman equation sets up a fixed-point condition for the value function.
%     \item The Bellman error quantifies the difference between the current $v$ and the Bellman update.
%     \item In approximate methods, we project the Bellman update onto the approximation space, yielding the \emph{Projected Bellman Error}.
%     \item The PBE can be measured via a weighted $\ell_2$-norm under $p^*(s)$, capturing how far $\hat{v}(w)$ is from its projected Bellman update.
%     \item Neither BEE nor PBE requires knowledge of the true value $v^*$; they only require the Bellman operator and (possibly) its samples.
%     \item This leads to stable iterative methods like LSTD or TD, which converge under appropriate conditions without ever using the exact $v^*$.
% \end{itemize}
...
\begin{tikzpicture}[scale=2, >=stealth, line join=round, line cap=round]
    ...
    \draw[fill] (PiBvW) circle (1pt)
         node[right] {\footnotesize $\Pi[\mathbb{B}\hat{v}(\mathbf{w})]$};
...
\begin{tikzpicture}[>=stealth, scale=1.2, line cap=round, line join=round]
    ...
    % Parabola
    % We can approximate a parabola with a plot or just a curved line.
